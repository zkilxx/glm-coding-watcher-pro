...
- **稳健**：500ms DOM 兜底检测，10 秒刷新，适合长期等待。
- **平衡（默认）**：200ms DOM 兜底检测，3 秒刷新。
- **冲刺**：100ms DOM 兜底检测，最快 1 秒刷新，最多 5 分钟后自动恢复平衡。

出现“购买人数过多”时使用 1、1、2、3、5、8 秒退避；发现明确限流或风控提示时使用 30、60、120 秒退避。高频刷新可能降低成功率或触发平台限制，请仅在明确的短时间窗口使用冲刺。

## 选择目标套餐

1. 打开 GLM Coding 页面后，在弹窗点击“刷新套餐列表”。
2. 同时勾选一个或多个可接受的订阅版本。
...
- `tabs`：读取当前标签页、向受支持页面发送状态并执行可选刷新。
